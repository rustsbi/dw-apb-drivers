...
/// External interrupt mode pad on port A.
...
impl<'a> EintPad<'a> {
    ...
    ///
    /// # Safety
    /// The requirements of [`crate::flex_pad::FlexPad::new`] and [`crate::flex_pad::FlexPad::into_eint`] apply.
    #[inline]
    ...
    pub fn set_event(&mut self, event: Event) {
        critical_section::with(
            #[inline(always)]
            |cs| ops::op_set_event(cs, &self.pad, event),
        );
    }
...
impl ErrorType for EintPad<'_> {
    type Error = Infallible;
}

impl InputPin for EintPad<'_> {
    #[inline(always)]
